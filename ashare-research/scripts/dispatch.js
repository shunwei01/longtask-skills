#!/usr/bin/env node
/**
 * dispatch.js — Concurrent subagent dispatcher for target analysis
 *
 * Usage:
 ...
 *   2. For each target: build prompt → launch subagent via pi -p → check output → update state
...
async function main() {
  ...
  const focus = env.ANALYSIS_FOCUS || '趋势与关键位';
  const segmentsDir = path.join(args.root, '.task-data', 'segments');
  const inputsDir = path.join(args.root, '.task-data', 'inputs');

  // Reset stale in_progress entries (interrupted runs) — otherwise they would
  // be silently skipped by the pending filter below.
  let resetCount = 0;
    ...
    if (entry.status === 'in_progress') {
      ...
      if (fs.existsSync(outputPath)) {
        try {
          ...
          if (out && out.target && out.target.name) {
            entry.status = 'done';
            console.log(`[dispatch] Recover in_progress→done: ${entry.id} (${entry.name})`);
            continue;
          }
        } catch {}
      }
      ...
      resetCount++;
      console.log(`[dispatch] Reset stale in_progress→pending: ${entry.id} (${entry.name})`);
    }
  }
  if (resetCount > 0 || manifest.some(e => e.status === 'done')) {
    writeManifest(manifestPath, manifest);
  }
  ...
  if (args.dryRun) {
    ...
    for (const task of tasks) {
      ...
      const prompt = buildPrompt(args.root, inputPath, outputPath, focus);
      console.log(`\n─── ${task.id} (${task.name}) ───`);
  ...
  async function runChunk(chunk) {
    ...
    console.log(`[dispatch] ▶ ${chunkId} (${chunk.name})`);
    ...
    try {
      ...
      await execAsync(cmd, {
        timeout: 10 * 60 * 1000,       // 10 min per target
        maxBuffer: 10 * 1024 * 1024,    // 10 MB
      });

      // Check output (Task Contract: file exists + valid JSON + required fields)
      if (fs.existsSync(outputPath)) {
        try {
          const out = JSON.parse(fs.readFileSync(outputPath, 'utf-8'));
          if (out && out.target && out.target.name) {
            updateStatus(manifestPath, chunkId, 'done');
            logEvent(args.root, 'chunk_done', { chunk: chunkId, target: out.target.name });
            console.log(`[dispatch] ✓ ${chunkId}: done (${out.target.name} ${out.period_change_pct > 0 ? '+' : ''}${out.period_change_pct}%)`);
          } else {
  ...
  console.log('[dispatch] Run merge.js next (Phase 3) to generate research report.');
}
